...
#extract uk map data
uk <- data.frame(map('worldHires','UK:Great Britain',plot=FALSE,xlim=c(-11,3), ylim=c(49,60.9))[c('x','y')])
...
ukmap <- ggplot(uk, aes(x, y),geom='path') +
  geom_polygon(colour='grey',fill = 'white') + 
  #pinpoint London town
  geom_point(data = london,aes(x,y,size=3,colour = 'red'), show_guide = FALSE) + 
  geom_text(data = london,aes(x,y),label='Where I \ncurrently\nlive      ',size=4,fontface='bold', hjust=1.2) +
  scale_y_continuous(name="Latitude") +
  scale_x_continuous(name="Longitude") +
  coord_map(project='azequalarea',orientation=c(51.5171,0.1062,0))

#use gridExtra's grid.arrange function to mimic base R's par(mfrow=c(1,2))
#i.e. put plots side by side
sidebysideplot <- grid.arrange(nzmap, ukmap, ncol=2)


```

```{r maps, fig.width=10, fig.height=5}
